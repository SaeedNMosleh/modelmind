...
export class PromptFooConfigGenerator {
  ...
  async generateConfig(
    ...
    const config: PromptFooConfig = {
      prompts: [{
        ...
        content: activeVersion.template
      }],
    ...
    }, 'Generated PromptFoo config object');
  ...
  private generateProviders(options: TestExecutionOptions): PromptFooConfig['providers'] {
    ...
    if (provider === 'openai') {
      providers.push(`openai:${model}:${JSON.stringify({
        temperature,
        ...(options.maxTokens && { max_tokens: options.maxTokens })
      })}`);
    } else if (provider === 'anthropic') {
      providers.push(`anthropic:${model}:${JSON.stringify({
        temperature,
        ...(options.maxTokens && { max_tokens: options.maxTokens })
      })}`);
    }
  ...
  async generateConfigWithFile(
    prompt: IPrompt,
    testCases: ITestCase[],
    options: TestExecutionOptions = {}
  ): Promise<{ config: PromptFooConfig; configPath: string }> {
    const { config } = await this.generateConfig(prompt, testCases, options);
    ...
    logger.info({
      ...
      testCaseCount: testCases.length,
      configPath
    }, 'Generated PromptFoo config file');

    return { config, configPath };
  }

  generateYaml(config: PromptFooConfig): string {
    const yaml = `# PromptFoo Configuration
# Generated by ModelMind Runtime Testing System

prompts:
${config.prompts.map(p => typeof p === 'string' 
  ? `  - "${p}"` 
  : `  - id: "${p.id || 'unknown'}"
    template: |
